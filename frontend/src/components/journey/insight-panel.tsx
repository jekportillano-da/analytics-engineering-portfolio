...
function WageAxisTick({ x = 0, y = 0, payload }: { x?: number; y?: number; payload?: { value?: string } }) {
  const words = payload?.value?.split(" ") ?? [];
  const lines = words.reduce<string[]>((result, word) => {
    const previous = result.at(-1) ?? "";
    if (`${previous} ${word}`.trim().length > 29) result.push(word);
    else result[result.length - 1] = `${previous} ${word}`.trim();
    return result;
  }, [""]);

  return <text fill="#a6b1bc" fontSize="11" textAnchor="end" x={x} y={y}>{lines.map((line, index) => <tspan dy={index === 0 ? "0.35em" : "1.15em"} key={`${line}-${index}`} x={x}>{line}</tspan>)}</text>;
}
...
function WageWorkspace({ data }: { data: WageInsightData }) {
  ...
  const chartHeight = Math.max(560, rows.length * 38 + 80);
  const definitions = Object.fromEntries(data.definitions.map((definition) => [definition.metric_id, definition]));
